...
Import-Csv $csvFile | ForEach-Object {
   ...
   try {
     ...
     if($change -eq "yes") {
        ...
        LogWrite "Send-MailMessage -To $email -from $originatorEmail -bcc $bcc -Subject $subject -Body $body -BodyAsHtml -encoding ([System.Text.Encoding]::UTF8) -Attachments $attachment -SmtpServer exchange.koeln.egetuerk.de"
        
        #  Sends the email to current userobject
        if(!$debug){Send-MailMessage -To $email -from $originatorEmail -bcc $bcc -Subject $subject -Body $body -BodyAsHtml -encoding ([System.Text.Encoding]::UTF8) -Attachments $attachment -SmtpServer $smtpServer}
     }
   } catch {
     #  Writes information in the log
